...
apt-get install net-tools traceroute coreutils
# sudo traceroute -T neuro.debian.net
traceroute -T neuro.debian.net
...
wget -O- http://neuro.debian.net/lists/${DISTRIB_CODENAME}.us-nh.full | tee /etc/apt/sources.list.d/neurodebian.sources.list
...
apt-cache policy           # What is actually available?
